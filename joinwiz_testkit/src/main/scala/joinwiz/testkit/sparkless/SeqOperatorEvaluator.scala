...
class SeqOperatorEvaluator[T, U](op: Operator) {
  def evaluate(left: T, right: U): Boolean = op match {
    case Equality(l: LTColumn[T, _, _], r: RTColumn[U, _]) => l(left) == r.expr(right)

    case Equality(l: LTColumn[T, _, _], r: Const[_]) => l(left) == r.value
    case Equality(l: Const[_], r: LTColumn[T, _, _]) => l.value == r(left)
