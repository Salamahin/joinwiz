...
trait EqualSyntax extends LowLevelEqualSyntax {
  ...
  implicit class OptionalLTCol2EqualSyntax[L, T](thisCol: LTCol2[L, Option[T]]) {
    ...
    def =:=[R](thatCol: LTCol2OptW[L, T]): Expr[L, R] = expr[L, R](thisCol.column === thatCol.wrapped.column)((l, _) => thisCol(l).isDefined && thatCol.wrapped(l) == thisCol(l))
    def =:=[R](thatCol: RTCol2OptW[R, T]): Expr[L, R] = expr[L, R](thisCol.column === thatCol.wrapped.column)((l, r) => thisCol(l).isDefined && thatCol.wrapped(r) == thisCol(l))
    def =:=[R](const: T): Expr[L, R]                  = expr[L, R](thisCol.column === lit(const))((l, _) => thisCol(l) contains const)
  }

  implicit class OptionalRTCol2EqualSyntax[R, T](thisCol: RTCol2[R, Option[T]]) {
    ...
    def =:=[L](thatCol: LTCol2OptW[L, T]): Expr[L, R] = expr[L, R](thisCol.column === thatCol.wrapped.column)((l, r) => thisCol(r).isDefined && thatCol.wrapped(l) == thisCol(r))
    def =:=[L](thatCol: RTCol2OptW[R, T]): Expr[L, R] = expr[L, R](thisCol.column === thatCol.wrapped.column)((_, r) => thisCol(r).isDefined && thatCol.wrapped(r) == thisCol(r))
