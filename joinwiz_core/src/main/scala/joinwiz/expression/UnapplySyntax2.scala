...
trait Tupled2[F[_, _, _]] {
  def left[L, R, A, B](t: F[L, R, (A, B)]): F[L, R, A]
  def right[L, R, A, B](t: F[L, R, (A, B)]): F[L, R, B]
}

object Tupled2 extends ApplyTColumnSyntax {
  implicit val applyToLeftTupled: Tupled2[LTColumn] = new Tupled2[LTColumn] {
  ...
  implicit val applyToRightTupled: Tupled2[RTColumn] = new Tupled2[RTColumn] {
...
trait UnapplySyntax2 {
  object wiz2 {
    def unapply[F[_, _, _]: Tupled2, L, R, A, B](tupled: F[L, R, (A, B)])(implicit t: Tupled2[F]): Option[(F[L, R, A], F[L, R, B])] = Some(t.left(tupled), t.right(tupled))
  }
}
